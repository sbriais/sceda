...
static int SCEDA_augment_flow_along_neg_cycle(SCEDA_Graph *g, SCEDA_Vertex *s, int (*capacity)(SCEDA_Edge *e, void *ctxt), void *cap_ctxt, int (*cost)(SCEDA_Edge *e, void *ctxt), void *cost_ctxt, SCEDA_HashMap *flow) {
  ...
  safe_call(SCEDA_queue_enqueue(queue, s));
  safe_call(SCEDA_queue_enqueue(queue, NULL));
  safe_call(SCEDA_hashset_add(in_queue, s));

  int i = 0;
  while((i < n) && (!SCEDA_queue_is_empty(queue))) {
    //    fprintf(stderr,"Phase %d\n", i);
    SCEDA_Vertex *u;
    //    fprintf(stderr,"dequeueing...");
    ...
    SCEDA_Edge *incoming_u = SCEDA_hashmap_get(incoming_edges, u);

    if((incoming_u != NULL) || (u == s)) {
      // out edges
      //      fprintf(stderr,"iterator over out edges\n");
	...
	SCEDA_Edge *incoming_v = SCEDA_hashmap_get(incoming_edges, v);
	boxed(int) rev_v = SCEDA_hashmap_get(reversed, v);
	int ce = cost(e, cost_ctxt);
	if((incoming_v == NULL) || (boxed_get(dist_u) + ce < boxed_get(dist_v))) {
      ...
      // in edges
      //      fprintf(stderr,"iterator over in edges\n");
  ...
  if((i < n) || (SCEDA_queue_is_empty(queue))) {
    //    fprintf(stderr,"no negative cycle\n");
  ...
  //  fprintf(stderr,"can augment flow along a negative cycle\n");
...

  {
    ...
    do {
      ...
      //      fprintf(stderr,"u = %s, e = (%s, %s), rev_e = %d\n",SCEDA_vertex_get_data(char *, u), SCEDA_vertex_get_data(char *, SCEDA_edge_source(e)), SCEDA_vertex_get_data(char *, SCEDA_edge_target(e)), boxed_get(rev_e));
  ...
  {
    //    fprintf(stderr,"will augment flow of %d\n", min_flow);
    SCEDA_Vertex *u = cycle;
    do {
      SCEDA_Edge *e = SCEDA_hashmap_get(incoming_edges, u);
      boxed(int) rev_e = SCEDA_hashmap_get(reversed, u);
...
SCEDA_HashMap *SCEDA_graph_min_cost_max_flow(SCEDA_Graph *g, SCEDA_Vertex *s, SCEDA_Vertex *t, int (*capacity)(SCEDA_Edge *e, void *ctxt), void *cap_ctxt, int (*cost)(SCEDA_Edge *e, void *ctxt), void *cost_ctxt) {
  ...
  while(SCEDA_augment_flow_along_neg_cycle(g, s, capacity, cap_ctxt, cost, cost_ctxt, flow)) {
  }
